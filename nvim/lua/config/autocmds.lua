...
-- 診断情報を /tmp/nvim_diagnostics.json に書き出し（WezTerm tabline連携用）
local diag_augroup = vim.api.nvim_create_augroup("diagnostics_bridge", { clear = true })
local diag_timer = nil
local diag_file = "/tmp/nvim_diagnostics.json"
...
vim.api.nvim_create_autocmd("DiagnosticChanged", {
  ...
  callback = function()
    ...
    diag_timer = vim.defer_fn(write_diagnostics, 500)
  end,
})
...
vim.api.nvim_create_autocmd("VimLeavePre", {
  ...
  callback = function()
    os.remove(diag_file)
    os.remove("/tmp/nvim_test_results.json")
  end,
})
